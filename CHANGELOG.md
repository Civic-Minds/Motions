...
- **Back button overlapping the page title**: On meeting, motion, and committee pages, the floating back arrow could land on top of the heading instead of beside it — the button and the page column both changed width at the same screen size, leaving it no room to float into. (A same-day follow-up fix replaced the workaround, which had hidden the floating button below 1280px wide and stacked it above the title instead — wasting a full row on ordinary desktop windows. Back now sits in a gutter reserved by the page's own padding, so it stays beside the title at any desktop width without depending on how much margin happens to exist outside the page column.)
