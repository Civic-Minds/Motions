...
- **The Map page's "mostly X and Y" topics now filter the homepage when clicked**: Each links to `/?topic=X`, matching the existing `?ward=` URL-filter pattern, and pre-selects the matching topic pill so the filter's visibly active, not just silently applied.
- **Fixed motion-map tooltips not wrapping long titles**: Leaflet's default tooltip CSS forces `white-space: nowrap`, which silently overrode the `max-width` on the tooltip content, so a long motion title just stretched the box across the map instead of wrapping onto a second line. Fixed on the new Map page and both existing motion-pin tooltips (Vancouver's mini-map, the ward detail map).
...
- **Fixed a too-tight gap above the featured Learn card**: The spacing wrapper used margin-based spacing, but the card is wrapped in a link (an inline element by default), so its top margin was silently ignored — switched to a flex gap, which isn't affected by the child's display type.
...
- **Vancouver election page's top summary now tracks the actual campaign calendar**: Before nominations close it shows the nomination period, then vote-by-mail applications, then voting places, instead of a single "where to vote" tile that stayed the same the whole campaign — including during the nomination period, when there's nothing to vote at yet.
- **Rewrote the Vancouver election page's voting section to stop duplicating the guide**: It repeated the "How Voting Works" guide almost line-for-line across three cards, without ever linking to it. Replaced with the same prominent "Non-partisan guide to voting" card Toronto's election page already has, plus one compact reminder card for the other ways to vote.
- **Fixed the mayor not being marked on Vancouver's Councillors page**: Detection compared the exact name against `jurisdiction.mayorName` ("Ken Sim"), but Vancouver's vote data stores the mayor as "Mayor Ken Sim" with the title baked into the name, so the match silently failed — no badge, no amber styling, not sorted first. Now matches regardless of which convention a city's data uses, and the displayed name drops the redundant "Mayor" prefix since the badge already says it.
- **About/Privacy/Terms moved to one URL each, outside any city**: They were duplicated at `/toronto/...` and `/vancouver/...` with identical content, which isn't right for pages that are about the project as a whole. They now live at bare `/about`, `/privacy`, `/terms`; the old jurisdiction-nested links redirect there. To avoid stranding a Vancouver visitor on a cityless page, the header logo on these pages remembers the last city visited (stored locally) and links back there instead of the generic homepage.
- **Fixed the broken "Vancouver council pages" link on Sources**: It pointed at a renamed/retired URL; updated to the current page.
- **Sources page now only shows the current city's sources**: It previously listed both Toronto's and Vancouver's sources regardless of which city you were browsing, plus the Maps entry that applies to both.
- **Relabeled the Sources card eyebrows from city name to source type**: They read "VANCOUVER" on every card (redundant now that the page is filtered, and easy to misread as labeling the city itself); now read "OPEN DATA," "COUNCIL PAGES," or "MAPS" instead, and each card gets an icon matching its actual type instead of a generic map icon on every card.
- **Removed a duplicate disclaimer from Sources**: It repeated Transparency's existing "these are Motions' interpretations" disclaimer almost verbatim, which is Transparency's job, not a source directory's.
- **Wrote real per-committee descriptions for Vancouver**: All 6 Vancouver committees (Council, Policy & Strategic Priorities, City Finance & Services, Public Hearing, Special Council, Auditor General Committee) showed the same generic fallback text; each now has an accurate, sourced description.
- **Aligned "Latest voting record" and "Last checked" onto the same line**: They sat at different heights depending on how much content was above them; both now pin to the bottom of their card.
- **Fixed "Latest voting record: Invalid Date" on Transparency**: The date comparison compared display-formatted strings ("Jul 22, 2026") instead of parsing them, so it picked the wrong motion entirely and then failed to parse it. Now compares real dates.
- **Fixed a duplicate link on Transparency's Sources card**: It linked to the same external Open Data URL twice (once inline, once as the button); the button now points to the fuller internal Sources page instead.
- **Explained how "notable" is actually determined, per city**: The Topics card previously just said significance scores exist; it now names the real factors (contested vote, outcome, debate time, topic weight) and is honest that Toronto and Vancouver use different formulas.
- **Tightened the "How It Works" card copy**: Shorter, more direct phrasing across all three cards instead of dense one-sentence paragraphs.
- **Made the Privacy page's wording city-neutral**: It described "your selected Toronto ward" even when viewed from Vancouver's routes; now describes the mechanism generically.
- **Fixed "last checked" never appearing on Transparency**: The metadata-writing step only lived inside the gated refresh job, so on the common case (no new meeting found, job skipped) it never ran — the site's "last checked" date has been silently missing since this feature was added. Added an ungated step that always records a check, independent of whether new data was found.
- **More homepage polish**: Centered and narrowed the hero/why-cards column (it was left-aligned with dead space beside it, not actually centered), removed the now-redundant "Choose a city" intro text above the map, softened the map's hover-to-zoom level, and added About/GitHub links to the homepage footer.
- **Added more cities to the map, dropped "Planned" language**: Verified each province's actual 2026 municipal election cycle (Ontario/BC/Manitoba have one, Alberta/Saskatchewan/Nova Scotia/Newfoundland don't) before adding Ottawa, Hamilton, Winnipeg, and Victoria as grey pins. Their cards now show their real election date instead of a "Planned" badge — that language implied a Motions coverage commitment these cities (other than Ottawa) don't actually have.
- **Narrowed the hero/why-cards column, kept the map full width**: The homepage's intro text and feature cards were stretching to the full 1400px content width; constrained just that section to a readable column while leaving the map at full width.
- **Hovering a map card now flies the map to that city**: Matches the ward map's hover behavior — moving off the card resets to the full Canada view. Guarded against the programmatic map move re-triggering the viewport filter and yanking the hovered card out of the list.
...
- **Simplified the Toronto voting guide header**: Removed the redundant “Back to election” link.
...
- **Footer branding remains visible**: The site name and description stay in the upper footer while navigation aligns with the homepage cards.
...
- **Vancouver motion addresses are now mapped**: Location-specific records receive high-confidence address pins during the Vancouver data refresh.
...
### Fixed
- **Councillor Profile Layout Alignment**: Restructured the profile header container on `lg` screens in [CouncillorProfile.jsx](file:///Users/ryan/Desktop/Mag/Tools/Council/Motions/src/components/CouncillorProfile.jsx) to use the identical column configuration (`lg:grid-cols-[220px_1fr_220px]` or `lg:grid-cols-[220px_1fr]`) as the body section below it. Separated the councillor identity details, the first 4 stat cards, and the 5th "2025 office spend" card into their respective aligned columns, fixing desktop layout misalignment and removing redundant fixed-width hacks on the identity card.
- **YourWardCard Navigation Click & Accessibility**: Changed the wrapper element of `YourWardCard` from `motion.button` to `motion.div`. Because the card contains nested interactive elements (`button` and `Link`), nesting them inside an outer `button` is invalid HTML, causing browsers to hoist the link to cover the entire card. Converting it to a `div` ensures clicking the card correctly navigates to the Ward details page while clicking "See more" goes to the Councillor profile. Also added full keyboard accessibility (`tabIndex={0}`, `role="button"`, custom `onKeyDown` handlers for Enter/Space, and focus-visible ring styles) to the card's `div` wrapper.
- **Dashboard Bento Grid Alignment & Header Renaming**: Standardized the bento row layout grid on `lg` screens to always use `lg:grid-cols-[200px_1fr_220px]` matching the main section columns below. Added the `YourWardCard` as the fallback in the left column when no committees are followed, preventing layout shifts and misalignments. Renamed duplicate "Your Ward" headers to "My Ward" (left column) and "Ward Motions" (middle column) to eliminate layout confusion.
- **Summary Generation Triviality Syncing**: Fixed a bug in `generate_summaries.js` where the `trivial` boolean flag was not updated when Gemini generated a new `significance` score. This caused newly summarized/rated motions to remain hidden from the "notable" filters on the dashboard even when their significance scores were high.
