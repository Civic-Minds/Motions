...
export default function MeetingsListView({ meetings = [] }) {
  ...
  const [agendaFilter, setAgendaFilter]       = useState(false);
  const [inCameraFilter, setInCameraFilter]   = useState(false);
  ...
  return (
    <div className="max-w-3xl mx-auto py-2 px-4 sm:px-6">
      ...
      {/* Single filter row */}
      <div className="flex items-center gap-1.5 mb-2 flex-nowrap overflow-x-auto pb-0.5">
        {/* Time */}
        {[
        ...
        {Object.keys(typeOptions).length > 2 && <span className="shrink-0 w-px h-4 bg-slate-200 mx-0.5" />}
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full appearance-none bg-slate-50 border border-slate-200 rounded-lg px-3 pr-8 py-2 text-xs font-medium text-slate-700 cursor-pointer"
                  >
                ...
                <div className="space-y-1">
                  <button
                    ...
                    className={cn(
                      ...
                      agendaFilter ? "bg-emerald-50 text-emerald-700" : "hover:bg-slate-50 text-slate-600"
                    )}
                  >
                    <FileText className="w-3.5 h-3.5 shrink-0" />
                    Agenda published
                    {agendaFilter && <span className="ml-auto text-emerald-500 text-[10px] font-bold">ON</span>}
                  </button>
                  <button
                    onClick={() => setInCameraFilter(v => !v)}
                    className={cn(
                      "w-full flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium transition-colors text-left",
                      inCameraFilter ? "bg-amber-50 text-amber-700" : "hover:bg-slate-50 text-slate-600"
                    )}
                  >
                    <Lock className="w-3.5 h-3.5 shrink-0" />
                    Has in-camera items
                    {inCameraFilter && <span className="ml-auto text-amber-500 text-[10px] font-bold">ON</span>}
                  </button>
                </div>
              </div>
