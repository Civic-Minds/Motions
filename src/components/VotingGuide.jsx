...
const RACES = [
  ['Mayor', 'Citywide'],
  ['City Councillor', 'Your ward'],
  ['School Board Trustee', 'Your ward'],
];
  ...
  return (
    ...
    >
      ...
      <section className="space-y-3">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <GuideCard icon={MapPin} title="Find your voting place">
            <p>Vote at any voting place during advance voting or on election day. Use MyVote, the City’s tool for finding your voting place — check again close to the date, since locations can change.</p>
          ...
          <GuideCard icon={CheckCircle2} title="Check your registration">
            <p>Use MyVote to confirm or update your info on the voters’ list, and get a digital voter information card — the same card the City mails before the election.</p>
            <p className="mt-2">You don’t need that card with you to vote, though.</p>
          </GuideCard>
        </div>
      </section>
      ...
      <section className="space-y-3">
        ...
        <GuideCard icon={Vote} title="Three races, one vote each">
          <div className="grid gap-2 sm:grid-cols-3">
            {RACES.map(([title, caption]) => <CivicMiniTile key={title} title={title} caption={caption} />)}
          </div>
      ...
      <section className="space-y-3">
        ...
        <CivicCard className="gap-3">
          <div className="grid gap-4 text-sm leading-relaxed text-slate-500 sm:grid-cols-3">
            <p><strong className="font-semibold text-slate-700">Check in.</strong> Bring ID — an election official confirms your info and hands you your ballots.</p>
            <p><strong className="font-semibold text-slate-700">Mark your ballot.</strong> Mark it in private, then place it in the ballot box or feed it into the scanner.</p>
            <p><strong className="font-semibold text-slate-700">Ask for help.</strong> Ask an election official for help, bring a friend, or request curbside voting or a transfer to a more accessible location.</p>
          </div>
        </CivicCard>
      </section>
      ...
      <section className="space-y-3">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <GuideCard icon={ShieldCheck} title="Bring identification">
            <p>Bring one piece of ID showing your name and a Toronto address. Your voter information card doesn’t count as ID.</p>
          ...
          <GuideCard icon={CheckCircle2} title="Time off to vote">
            <p>You’re entitled to three consecutive hours free to vote on election day — not three extra hours. If you work 10 a.m.–6 p.m., your employer could let you leave at 5 p.m. so you have until polls close at 8 p.m.</p>
          </GuideCard>
        </div>
        <CivicCard className="gap-3">
          <div className="grid gap-4 text-sm leading-relaxed text-slate-500 sm:grid-cols-2">
            <ul className="space-y-3">
              <li>Voter Assist Terminals, which let you mark your ballot independently, are available at all advance voting places and two per ward on election day.</li>
              <li>Translation and interpretation support is available through Toronto Elections and 311.</li>
            </ul>
            <ul className="space-y-3">
              <li>Voters without a permanent address can vote using a shelter, drop-in centre, or MyVote information.</li>
              <li>Voters held in custody awaiting trial can ask detention-centre staff about proxy or mail-in voting.</li>
            </ul>
          </div>
      ...
      <section className="space-y-3">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <GuideCard icon={Mail} title="Vote by mail">
            <p>Eligible voters can vote by mail. Use MyVote or contact Toronto Elections for the current application window and deadline.</p>
          ...
          <GuideCard icon={CheckCircle2} title="Vote by proxy">
            <p>Can’t vote on any available day? Appoint one eligible Toronto voter to vote on your behalf.</p>
      ...
      <section className="border-t border-slate-200 pt-6 text-sm text-slate-500">
        <p>
          This guide is practical and non-partisan. Confirm dates, requirements, voting places, and accessibility details with <a className={inlineLink} href="https://www.toronto.ca/city-government/elections/" target="_blank" rel="noopener noreferrer">Toronto Elections</a> before voting.
        </p>
      </section>
    </VotingGuideShell>
  );
