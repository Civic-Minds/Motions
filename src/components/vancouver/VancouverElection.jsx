...
import { CivicCard, CivicSectionLabel, ElectionSummaryCard, GuideCallout, OfficialLink } from '../ui/CivicCard';
import ElectionResultCards from '../election/ElectionResultCards';
import { useAppContext } from '../../contexts/AppContext';
import { isOnOrAfter, formatElectionDateFull, disclaimerText } from '../../utils/electionDate';
...
export default function VancouverElection() {
  ...
  return (
    <div className="flex flex-col space-y-4 pb-20">
      ...
      <section className="grid grid-cols-2 gap-3 items-stretch lg:grid-cols-6">
        <div className="flex flex-col gap-1.5 lg:col-span-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            <ElectionSummaryCard pill="My city" pillClass="bg-slate-100 text-slate-600" footer={<Link to="/councillors" className="text-[9px] font-semibold text-[#004a99]">See council</Link>}>
            ...
            <ElectionSummaryCard pill="Candidates" pillClass="bg-blue-50 text-[#004a99]" footer={<a href={CANDIDATE_INFO} target="_blank" rel="noopener noreferrer" className="text-[9px] font-semibold text-[#004a99]">Candidate info ↗</a>}>
        ...
        <div className="flex flex-col gap-1.5 lg:col-span-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            {infoPhase === 'closed' ? (
              <ElectionSummaryCard pill="Voting closed" pillClass="bg-slate-100 text-slate-600" footer={<Link to="/councillors" className="text-[9px] font-semibold text-[#004a99]">See council</Link>}>
              ...
              <ElectionSummaryCard pill="Nomination period" pillClass="bg-blue-50 text-[#004a99]" footer={<a href={CANDIDATE_INFO} target="_blank" rel="noopener noreferrer" className="whitespace-nowrap text-[9px] font-semibold text-[#004a99]">See who’s running ↗</a>}>
              ...
              <ElectionSummaryCard pill="Vote by mail" pillClass="bg-blue-50 text-[#004a99]" footer={<a href={VOTE_BY_MAIL} target="_blank" rel="noopener noreferrer" className="whitespace-nowrap text-[9px] font-semibold text-[#004a99]">Apply ↗</a>}>
                <p className="flex-1 text-xs font-semibold leading-snug text-slate-800">Vote-by-mail applications are open.</p>
              </ElectionSummaryCard>
            ) : (
              <ElectionSummaryCard pill="Where to vote" pillClass="bg-blue-50 text-[#004a99]" footer={<a href={VOTING_PLACES} target="_blank" rel="noopener noreferrer" className="text-[9px] font-semibold text-[#004a99]">Find a place ↗</a>}>
                <p className="flex-1 text-xs font-semibold leading-snug text-slate-800">Vote anywhere in Vancouver.</p>
              </ElectionSummaryCard>
            )}
            <ElectionSummaryCard pill="How to vote" footer={<Link to="/learn/how-voting-works" className="whitespace-nowrap text-[9px] font-semibold text-[#004a99]">Our guide ↗</Link>}>
        ...
        <div className="flex flex-col gap-1.5 lg:col-span-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            {electionOver ? (
              ...
              <>
                <ElectionSummaryCard pill="Election day" pillClass="bg-blue-50 text-[#004a99]" footer={<a href={OFFICIAL} target="_blank" rel="noopener noreferrer" className="whitespace-nowrap text-[9px] font-semibold text-[#004a99]">Official details ↗</a>}>
                  <p className="flex-1 text-xs font-semibold leading-snug text-slate-800">October 17, 2026, from 8 a.m. to 8 p.m.</p>
                </ElectionSummaryCard>
                <ElectionSummaryCard pill="Advance voting" footer={<a href={VOTING_PLACES} target="_blank" rel="noopener noreferrer" className="whitespace-nowrap text-[9px] font-semibold text-[#004a99]">See locations ↗</a>}>
      ...
      <p className="text-xs leading-relaxed text-slate-400">{disclaimerText(jurisdiction)} Official election information takes precedence and may change.</p>
    </div>
  );
}
